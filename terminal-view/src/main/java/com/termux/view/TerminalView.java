...
public final class TerminalView extends View {
    ...
    /**
     * VibeTerm: the IME composing (preedit) text, e.g. pinyin being typed before a candidate is
     * chosen. It is never sent to the remote - it is drawn locally as an overlay at the cursor
     * position in {@link #onDraw(Canvas)}, and cleared when the IME commits or aborts composition.
     */
    private String mImePreedit;
    private final Paint mImePreeditPaint = new Paint();
    private final RectF mImePreeditRect = new RectF();
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        // VibeTerm patch: upstream termux declares TYPE_NULL here ("raw key events only"),
        // which makes IMEs fall back to a latin-only mode without composing text support -
        // the reason CJK input never works in terminal apps. We instead always declare a real
        ...
        return new BaseInputConnection(this, true) {
            ...
            /** Update the preedit overlay from the current composing span of the editable. */
            void updateImePreedit() {
                ...
                if (start != -1 && end != -1 && end > start) {
                    preedit = content.subSequence(start, end).toString();
                }
                setImePreedit(preedit);
            }
            ...
            public boolean performEditorAction(int actionCode) {
                if (TERMINAL_VIEW_KEY_LOGGING_ENABLED)
                    mClient.logInfo(LOG_TAG, "IME: performEditorAction(" + actionCode + ")");
                // Some IMEs send the enter key as an editor action instead of commitText("\n").
                if (mTermSession != null) mTermSession.write("\r");
                return true;
            }
    ...
    protected void onDraw(Canvas canvas) {
        if (mEmulator == null) {
            ...
            drawImePreedit(canvas);
        }
    }

    /** VibeTerm: update the IME preedit overlay text and repaint. Called from the InputConnection. */
    void setImePreedit(String preedit) {
        if (preedit != null && preedit.isEmpty()) preedit = null;
        if (!TextUtils.equals(preedit, mImePreedit)) {
            mImePreedit = preedit;
            invalidate();
        }
    }

    /** Draw the IME composing text as a floating box anchored at the cursor cell. */
    private void drawImePreedit(Canvas canvas) {
        if (mImePreedit == null || mRenderer == null) return;

        final float pad = mRenderer.mFontWidth * 0.4f;
        mImePreeditPaint.setTypeface(mRenderer.mTypeface);
        mImePreeditPaint.setTextSize(mRenderer.mTextSize);
        mImePreeditPaint.setAntiAlias(true);

        float textWidth = mImePreeditPaint.measureText(mImePreedit);
        int cursorRow = mEmulator.getCursorRow() - mTopRow;
        // If scrolled back beyond the cursor row, pin the overlay to the bottom of the view.
        if (cursorRow < 0 || cursorRow >= mEmulator.mRows) cursorRow = mEmulator.mRows - 1;

        float cellBottom = mRenderer.mFontLineSpacingAndAscent + (cursorRow + 1) * mRenderer.mFontLineSpacing;
        float x = mEmulator.getCursorCol() * mRenderer.mFontWidth;
        float boxWidth = textWidth + 2 * pad;
        if (x + boxWidth > getWidth()) x = Math.max(0, getWidth() - boxWidth);

        mImePreeditRect.set(x, cellBottom - mRenderer.mFontLineSpacing - pad, x + boxWidth, cellBottom + pad * 0.5f);
        mImePreeditPaint.setColor(0xF2333333);
        float corner = pad * 0.75f;
        canvas.drawRoundRect(mImePreeditRect, corner, corner, mImePreeditPaint);

        mImePreeditPaint.setColor(0xFFFFFFFF);
        mImePreeditPaint.setUnderlineText(true);
        canvas.drawText(mImePreedit, x + pad, cellBottom - mRenderer.mFontLineSpacingAndAscent, mImePreeditPaint);
        mImePreeditPaint.setUnderlineText(false);
    }

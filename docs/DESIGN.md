...
- [x] **M5 多窗口**:标签页多会话(vt1/vt2 独立)+ 附加键条逐键验证(cat -v 实证:Esc=^[、⇧Tab=^[[Z、↑=^[[A、→=^[[C、PgUp=^[[5~、Ctrl闩锁+c=^C)(2026-08-22)
- [x] **M6 多服务器**:主机管理 UI、Keystore 密码存储、TOFU 指纹(随 M2 已交付并在真机/模拟器使用)
- [x] **M7 完成通知**:bell(终端响铃)与忙碌后静默(任务可能已完成)双信号均在模拟器实测命中,分屏可见面板不打扰(2026-08-22)
- [x] **M8 分屏**:≥840dp 出现分屏按钮;双栏独立会话、焦点边框、输入/附加键条按焦点路由,模拟器横屏实测通过(2026-08-22)
- [x] **M9 打磨**(2026-08-22):内置 JetBrains Mono NL 字体(204KB,OFL 许可,CJK 走系统回退);GitHub-Dark 终端 ANSI 配色(TerminalColors.COLOR_SCHEME 全局应用,与 App 主题统一);主机列表卡片化+终端风格空状态;胶囊式会话标签;设置对话框(响铃/静默通知开关);自适应启动图标;字号双指缩放+持久化

每个里程碑的验收标准:真机(或至少模拟器)可演示对应能力,且此前所有里程碑不回归。

### v0.2 vibe coding 增强(2026-08-22,模拟器全量验证)

- [x] **锁屏批准**:静默检测命中后读取屏幕末尾文本,识别确认提示(y/n、Do you want、选项列表等)时通知标题改「可能在等你确认」;通知带「✅确认(回车)」「✋打断(Esc)」按钮,经 NotificationActionReceiver 直接写入会话。**安全**:默认要求先解锁认证(API 31+ 用 `setAuthenticationRequired`,指纹一碰即过);API 26–30 无法强制认证,此时默认**不显示**这些直连按钮,只保留点击进 App(必经 keyguard)——除非用户在设置里显式开启「锁屏免解锁批准」
